defmodule APITest do
  ...
  test "response body value types are converted" do
    assert user = API.get_user!("cookiezi")
  ...
  test "get_user" do
    assert {:ok, user} = API.get_user("cookiezi")
    assert is_map(user)
    assert Map.get(user, :user_id) === 124_493

    assert {:ok, user} = API.get_user(124_493)
    assert is_map(user)
    assert Map.get(user, :user_id) === 124_493
    ...
    assert user = API.get_user!(124_493)
    assert is_map(user)
    assert Map.get(user, :user_id) === 124_493
  end

  test "get_scores" do
    assert {:ok, scores} = API.get_scores(129_891)
    assert is_list(scores)
    assert length(scores) === 50
    assert Enum.all?(scores, fn s -> Map.has_key?(s, :score_id) end)

    assert scores = API.get_scores!(129_891)
    assert is_list(scores)
    assert length(scores) === 50
    assert Enum.all?(scores, fn s -> Map.has_key?(s, :score_id) end)
  end
  ...
  test "get_user_best" do
    assert {:ok, user_best} = API.get_user_best("cookiezi")
    ...
    assert {:ok, user_best} = API.get_user_best(124_493)
    assert is_list(user_best)
    assert length(user_best) === 10
    ...
    assert user_best = API.get_user_best!("cookiezi")
    assert is_list(user_best)
    assert length(user_best) === 10
    ...
    assert user_best = API.get_user_best!(124_493)
    assert is_list(user_best)
    assert length(user_best) === 10
  ...
  test "get_user_recent" do
    ...
    assert {:ok, user_recent} = API.get_user_recent("cookiezi")
    ...
    assert {:ok, user_recent} = API.get_user_recent(124_493)
    assert is_list(user_recent)
    ...
    assert user_recent = API.get_user_recent!("cookiezi")
    assert is_list(user_recent)
    ...
    assert user_recent = API.get_user_recent!(124_493)
    assert is_list(user_recent)
  ...
  test "get_replay" do
    assert {:ok, replay} = API.get_replay(129_891, "cookiezi", 0)
    assert is_map(replay)
    assert is_binary(replay.content)
    assert replay.encoding === "base64"

    assert replay = API.get_replay!(129_891, 124_493, 0)
    assert is_map(replay)
    assert is_binary(replay.content)
    assert replay.encoding === "base64"
  end
